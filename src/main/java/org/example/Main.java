    ...
    public class Main {
        ...
        // Global trackers for Spearman Weights (Used by the Refactoring Engine)
        private static double globalWeightTime = 0.5;
        private static double globalWeightMem = 0.5;

        // Thesis Calibration Variables (Loaded from Data Lake)
        private static double THESIS_WEIGHT_TIME = 0.5;
        private static double THESIS_WEIGHT_MEM = 0.5;
        private static double THESIS_Q1 = 0.0;
        private static double THESIS_Q3 = 100.0;
        private static boolean DATA_LAKE_READY = false;

        public static void main(String[] args) {
            SwingUtilities.invokeLater(Main::createAndShowGUI);
        }

        private static void initializeDataLake(JTextPane consolePane) {
            ...
            try {
                ...
                if(times.size() > 5) {
                    // 1. Calculate Universal Spearman Weights
                    ...
                    // Math.abs() acts as the Absolute Value bars | | from the formula.
                    // spearman.correlation represents the Greek letter Rho (ρ).
                    ...
                    //(|ρ_time,energy| + |ρ_memory,energy|)
                    double totalCorr = timeCorr + memCorr;

                    THESIS_WEIGHT_TIME = timeCorr / totalCorr;
                    THESIS_WEIGHT_MEM = memCorr / totalCorr;
                    // This calculates W_time and W_memory
                    globalWeightTime = THESIS_WEIGHT_TIME;
                    globalWeightMem = THESIS_WEIGHT_MEM;

                    // 2. Calculate Universal Quartiles
                    ...
                    for (int i = 1; i < lines.size(); i++) {
                        ...
                        if (!targetSmell.contains("Clean") && !targetSmell.contains("Floor") && !targetSmell.contains("Ceiling") && !targetSmell.contains("Baseline")) {
                            double normTime = Math.max(0, Math.min(1.0, Double.parseDouble(parts[4]) / CEILING_TIME));
                            double normMem = Math.max(0, Math.min(1.0, Double.parseDouble(parts[5]) / CEILING_MEMORY));
                            double score = ((normTime * THESIS_WEIGHT_TIME) + (normMem * THESIS_WEIGHT_MEM)) * 100.0;
                            dirtyPhase2Scores.add(score);
                        }
                    }

                    Collections.sort(dirtyPhase2Scores);
                    THESIS_Q1 = calculatePercentile(dirtyPhase2Scores, 25.0);
                    THESIS_Q3 = calculatePercentile(dirtyPhase2Scores, 75.0);
                    DATA_LAKE_READY = true;
                    logToConsole(consolePane, "[SYSTEM] Thesis Data Lake Loaded. Universal Weights & Quartiles Locked.\n");
                }
            } catch (Exception e) {
                System.out.println("Data lake init failed.");
            }
        }
        ...
        private static void createAndShowGUI() {
            ...
            quartileBtn.setBackground(new Color(80, 250, 123));
            runEisBtn.setForeground(new Color(30, 30, 30));
            ...
            JButton clearDataBtn = new JButton("\uD83D\uDDD1 Clear Results User Data");
            ...
            quartileBtn.addActionListener(e -> {
                ...
                try {
                    ...
                    for (int i = 1; i < lines.size(); i++) {
                        ...
                        if (parts.length >= 11) {
                            try {
                                double score = Double.parseDouble(parts[9]);
                                validRows.add(parts);
                    ...
                    for (String[] row : validRows) {
                        double score = Double.parseDouble(row[9]);
                        String qRank;
                        String colorTag;

                        String absoluteTier = "GREEN";
                        if (score > 20.00) absoluteTier = "YELLOW";
                        if (score > 60.00) absoluteTier = "RED";
                        ...
                        double executionTime = Double.parseDouble(row[4]);
                        double memoryMB = Double.parseDouble(row[5]);
                        double joules = Double.parseDouble(row[6]);

                        String targetSmell = row[2];
                        String insight = "";

                        if (targetSmell.toLowerCase().contains("dirty")
                                || (!targetSmell.toLowerCase().contains("clean")
                                && !targetSmell.toLowerCase().contains("floor")
                                && !targetSmell.toLowerCase().contains("ceiling"))) {

                            insight = DynamicRefactoringEngine.generateInsight(
                                    targetSmell,
                                    row[1],
                                    joules,
                                    executionTime,
                        ...
                        String csvOutput = String.format("%s,%s,%s,%s,%s,%s", row[0], row[1], targetSmell, row[3], row[9], qRank);
                        String shortMethod = row[3].substring(row[3].lastIndexOf(".") + 1);

                        String consoleOutput = String.format("   [%-28s] EIS: %05.2f | %s%s\n", shortMethod, score, colorTag, insight);
        ...
        public static void logToConsole(JTextPane pane, String textBlock) {
            ...
            for (String line : lines) {
                Color c = new Color(248, 248, 242); // Default White/Off-White
                ...
                // Check for YELLOW
                else if (line.contains("[YELLOW") || line.contains("WARNING") || line.contains("CODE SMELLS FOUND") || line.contains("• Yellow")) {
                ...
                // Check for Dark Purple (Headers/Borders)
                else if (line.contains("===") || line.contains("---")) {
                    c = new Color(98, 114, 164);
                }
                // Check for Light Purple (Main System Tags)
                else if (line.contains("GREEN JAVA") || line.contains("Config:")) {
                    c = new Color(189, 147, 249);
                }
                // Check for Light Blue (Data Labels & Sub-Headers)
                else if (line.contains("Primary Driver:") ||
                        line.contains("Joules/op:") ||
                        line.contains("Memory:") ||
                        line.contains("Exec Time:") ||
                        line.contains("Absolute Rank:") ||
                        line.contains("Relative Rank:") ||
                        line.contains("SYNTHESIS") ||
                        line.contains("DYNAMIC PROFILING") ||
                        line.contains("STATIC ANALYSIS") ||
                        line.contains("Target Smell:") ||
                        line.contains("Location:")) {
                    c = new Color(139, 233, 253);
                }
                // Note: REFACTORING BLUEPRINT, Problem, Suggestion, and Result
                // are no longer in the Light Blue block, so they will default to White!
            ...
            currentWorker = new SwingWorker<>() {
                ...
                protected Void doInBackground() {
                    ...
                    try {
                        ...
                        sonarCmd.add("-Dsonar.exclusions=target/**"); // FIXED: SONARQUBE DIRECTORY FILTER
                        ...
                        publishLog("\n[GREEN JAVA] Fetching Structural Analysis Report from SonarQube...\n");
                        Thread.sleep(3000);
                        ...
                        if (exitCode == 0 && !isCancelled()) {
                            ...
                            if(DATA_LAKE_READY) {
                                publishLog("\n[GREEN JAVA] Evaluating run against Thesis Data Lake...\n");
                                final double CEILING_TIME = 0.743411;
                                final double CEILING_MEMORY = 156448;
                                double normTime = Math.max(0, Math.min(1.0, jmhScoreSeconds / CEILING_TIME));
                                double normMem = Math.max(0, Math.min(1.0, allocatedMemoryMB / CEILING_MEMORY));
                                double currentEis = ((normTime * THESIS_WEIGHT_TIME) + (normMem * THESIS_WEIGHT_MEM)) * 100.0;

                                String absoluteTier = "GREEN";
                                if (currentEis > 20.00) absoluteTier = "YELLOW";
                                if (currentEis > 60.00) absoluteTier = "RED";

                                String dynamicTier = "GREEN_(Q1)";
                                if(currentEis >= THESIS_Q3) dynamicTier = "RED_(Q4)";
                                else if(currentEis >= THESIS_Q1) dynamicTier = "YELLOW_(Q2-Q3)";

                                String insight = "";
                                if (extractedSmell.toLowerCase().contains("dirty")
                                        || (!extractedSmell.toLowerCase().contains("clean")
                                        && !extractedSmell.toLowerCase().contains("floor")
                                        && !extractedSmell.toLowerCase().contains("ceiling"))) {
                                ...
                                publishLog("[WARNING] Data Lake not established. Restart application to load results.csv.\n");
                            }
        ...
        private static String calculateAndGetEISReport() {
            ...
            try {
                ...
                globalWeightTime = weightTime;
                globalWeightMem = weightMem;
                ...
                try (PrintWriter out = new PrintWriter(new FileWriter(reportFile))) {
                    out.println("Timestamp,ProjectName,TargetSmell,MethodName,RawTime,RawMem,RawJoules,NormTime,NormMemory,Final_EIS_Score,Category");
                    ...
                    for (String[] row : dataRows) {
                        ...
                        double rawJoules = Double.parseDouble(row[6]);

                        double normTime = Math.max(0, Math.min(1.0, rawTime / CEILING_TIME));
                        double normMem = Math.max(0, Math.min(1.0, rawMem / CEILING_MEMORY));
                        ...
                        out.printf("%s,%s,%s,%s,%.6f,%.2f,%.6f,%.4f,%.4f,%.2f,%s\n",
                                row[0], row[1], targetSmell, methodName, rawTime, rawMem, rawJoules, normTime, normMem, finalEIS, category);
